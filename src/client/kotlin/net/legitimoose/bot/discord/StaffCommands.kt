...
class StaffCommands(override val event: SlashCommandInteractionEvent, val command: String) : Command {
    override fun onCommandReceived() {
        if (event.guild?.id != LegitimooseBot.config.getOrDefault("discordGuildId", "1311574348989071440")) {
        ...
        when (command) {
            ...
            "help" -> {
                ...
                event.hook.sendMessage("Unknown command: `$command`. Use `/help` for a list of commands.").queue()
            }
            //TODO: Figure out how to login (maybe with headlessmc api???? or recreate headlessmc behavior+replace the first value in .accounts file) OR make a headlessmc plugin to handle this
