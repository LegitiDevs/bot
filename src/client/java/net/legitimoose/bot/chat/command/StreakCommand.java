    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(LiteralArgumentBuilder.<CommandSource>literal("streak")
                .then(LiteralArgumentBuilder.<CommandSource>literal("on").executes(context -> {
                    Player player = Database.getPlayers()
                            .find(eq("name", context.getSource().username()))
                            .first();
                    assert player != null;
                    if (player.streak().notifications() == true) {
                        ...
                        Database.getPlayers()
                                .updateOne(
                                        eq("name", context.getSource().username()), Updates.set("streak.notify", true));
                ...
                .then(LiteralArgumentBuilder.<CommandSource>literal("off").executes(context -> {
                    ...
                    if (player.streak().notifications() == false) {
                        ...
                        Database.getPlayers()
                                .updateOne(
                                        eq("name", context.getSource().username()),
                                        Updates.set("streak.notify", false));
                ...
                .then(RequiredArgumentBuilder.<CommandSource, String>argument("username", StringArgumentType.string())
                        .executes(context -> {
                            ...
                            context.getSource()
                                    .sendMessage(username + "'s current login streak is "
                                            + player.streak().days() + " day(s)");
                            return Command.SINGLE_SUCCESS;
                        }))
                .then(LiteralArgumentBuilder.<CommandSource>literal("leaderboard")
                        .executes(context -> {
                ...
                .executes(context -> {
                    ...
                    assert player != null;
                    context.getSource()
                            .sendMessage("Your current login streak is "
                                    + player.streak().days() + " day(s)");
                    return Command.SINGLE_SUCCESS;
                }));
    }
