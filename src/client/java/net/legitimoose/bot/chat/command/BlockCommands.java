        ...
        dispatcher.register(LiteralArgumentBuilder.<CommandSource>literal("block")
                .then(RequiredArgumentBuilder.<CommandSource, String>argument("username", StringArgumentType.string())
                        .executes(context -> {
                            ...
                            Database.getPlayers()
                                    .updateOne(eq("name", source.username()), Updates.set("blocked", List.of(blocked)));
                ...
                .then(LiteralArgumentBuilder.<CommandSource>literal("list").executes(context -> {
                    CommandSource source = context.getSource();
                    List<String> blockedPlayers = Database.getPlayers()
                            .find(eq("name", source.username()))
                            .first()
                            .blocked();
        ...
        dispatcher.register(LiteralArgumentBuilder.<CommandSource>literal("unblock")
                .then(RequiredArgumentBuilder.<CommandSource, String>argument("username", StringArgumentType.string())
                        .executes(context -> {
                            ...
                            String blocked = context.getArgument("username", String.class);
                            Database.getPlayers()
                                    .updateOne(eq("name", source.username()), Updates.pull("blocked", blocked));
