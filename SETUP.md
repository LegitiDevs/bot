...
Once installed, on Linux you can use the commands `sudo systemctl start mongod` and `sudo systemctl stop mongod` to start and stop the database.

If you want to change the URI for the database you can edit the field in the 
config (at `run/config/legitimoosebot.yml`) named `mongoUri`
...
you can generate a new token by going to Overview>Bot and clicking `Reset Token`
...
make sure `Developer Mode` is checked. This makes sure that you can see the `Copy Server Id` menu item.
